"""
Final Ensemble - Focus on fuel-specific patterns and quantile ranking
"""
...
# =============================================================================
# FEATURE ENGINEERING - FOCUS ON FUEL-SPECIFIC PATTERNS
# =============================================================================
...
def engineer_features(df, train_df=None, is_train=True):
    ...
    # CRITICAL: Fuel-specific patterns from EDA
    # Coal/Gas/Oil: smaller + older = underperforming
    ...
    # Hydro: LARGER = underperforming (OPPOSITE!)
    df['hydro_large'] = df['is_hydro'] * df['capacity_log_mw']
    df['hydro_large_rank'] = df['is_hydro'] * df['capacity_log_mw']  # Will be ranked separately

    # Waste: larger + old = underperforming
    df['waste_large_old'] = df['is_waste'] * df['capacity_log_mw'] * df['plant_age']

    # Solar/Wind: small = underperforming (age doesn't matter)
    df['solar_wind_small'] = (df['is_solar'] + df['is_wind']) * (5 - df['capacity_log_mw'])

    # Combined fuel interaction
    df['fuel_age_cap'] = df['primary_fuel'].map({
        'Gas': 1, 'Coal': 1, 'Oil': 1,  # Fossil: old + small bad
        'Hydro': -1,  # Hydro: large bad (opposite)
        'Solar': 0, 'Wind': 0,  # Solar/Wind: small bad
        'Waste': 2,  # Waste: large + old bad
        'Nuclear': 0, 'Biomass': 0, 'Other': 0
    }).fillna(0) * df['plant_age'] * df['capacity_log_mw']

    # Z-score within fuel (KEY for quantile-based target)
    if is_train:
    ...
    df['very_small'] = (df['capacity_log_mw'] < df['capacity_log_mw'].quantile(0.1)).astype(int)
    df['very_large'] = (df['capacity_log_mw'] > df['capacity_log_mw'].quantile(0.9)).astype(int)
    df['very_old'] = (df['plant_age'] > df['plant_age'].quantile(0.9)).astype(int)
...
# High-value combos from EDA
combos = [
    ['primary_fuel', 'other_fuel1'],  # Highest variance grouping
...
num_features = [
    ...
    'hydro_large', 'waste_large_old', 'solar_wind_small', 'fuel_age_cap',
    ...
    'very_small', 'very_large', 'very_old',
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    model = lgb.train(
        lgb_params, tr_data, num_boost_round=2000,
        valid_sets=[val_data],
...
print(f"LightGBM: ROC={roc_auc_score(y, lgb_oof):.4f}, AP={average_precision_score(y, lgb_oof):.4f}, Comp={composite(y, lgb_oof):.4f}", flush=True)
...
print(f"CatBoost: ROC={roc_auc_score(y, cb_oof):.4f}, AP={average_precision_score(y, cb_oof):.4f}, Comp={composite(y, cb_oof):.4f}", flush=True)
...
best_score = 0
best_w = 0.5
for w in np.arange(0.5, 0.8, 0.05):
