"""
Advanced Ensemble - 3 Models + Advanced Stacking
Target: Push past 0.80
"""
...
def engineer_features(df, train_df=None, is_train=True):
    ...
    # Fuel-specific patterns (CRITICAL from EDA!)
    ...
    # More granular fuel interactions
    df['gas_old'] = df['is_gas'] * df['plant_age']
    df['coal_old'] = df['is_coal'] * df['plant_age']
    df['oil_old'] = df['is_oil'] * df['plant_age']
...
print(f"LightGBM: Comp={composite(y, lgb_oof):.4f}", flush=True)
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    model = xgb.train(
        xgb_params, tr_data, num_boost_round=3000,
        evals=[(val_data, 'val')],
...
print(f"XGBoost: Comp={composite(y, xgb_oof):.4f}", flush=True)
...
print(f"CatBoost: Comp={composite(y, cb_oof):.4f}", flush=True)
...
meta_train = np.column_stack([lgb_oof, xgb_oof, cb_oof])
meta_test = np.column_stack([lgb_test, xgb_test, cb_test])
...
# Simple average
avg_oof = (lgb_oof + xgb_oof + cb_oof) / 3
avg_test = (lgb_test + xgb_test + cb_test) / 3
print(f"Simple Average: Comp={composite(y, avg_oof):.4f}", flush=True)
...
for w1 in [0.3, 0.4, 0.5, 0.6]:
    for w2 in [0.2, 0.3, 0.4]:
        ...
        if w3 < 0.1:
            continue
        weighted = w1 * lgb_oof + w2 * xgb_oof + w3 * cb_oof
        score = composite(y, weighted)
        if score > best_score:
            best_score = score
            best_weights = (w1, w2, w3)

print(f"Best Weighted: Comp={best_score:.4f}, Weights={best_weights}", flush=True)
...
if best_score > final_comp:
    final_oof = best_weights[0] * lgb_oof + best_weights[1] * xgb_oof + best_weights[2] * cb_oof
    final_test = best_weights[0] * lgb_test + best_weights[1] * xgb_test + best_weights[2] * cb_test
    final_comp = best_score
