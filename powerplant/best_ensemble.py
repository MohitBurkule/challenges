"""
Best Ensemble - Push for maximum score
"""
...
print("\n" + "="*60, flush=True)
print("LIGHTGBM (Best Params)", flush=True)
print("="*60, flush=True)

# Best params from previous run + slight tuning
lgb_params = {
    'objective': 'binary',
    'metric': ['auc'],
    'boosting_type': 'gbdt',
...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    model = lgb.train(
        lgb_params, tr_data, num_boost_round=3000,  # More rounds
        valid_sets=[val_data],
...
# LightGBM only
lgb_only_comp = composite(y, lgb_oof)
print(f"LightGBM only: Comp={lgb_only_comp:.4f}", flush=True)

# Weighted ensemble
best_score = lgb_only_comp
best_w = 1.0
for w in np.arange(0.85, 1.0, 0.01):
...
if best_w < 1.0:
    print(f"Best ensemble: {best_w:.2f} LGB + {1-best_w:.2f} CB = {best_score:.4f}", flush=True)
else:
    print(f"LightGBM only is best: {lgb_only_comp:.4f}", flush=True)
